...
You can pin resources to always be visible on top of the {doctitle}.

Drag and drop to the top area to pin. Drag outside the pinned area to unpin.

==== Pins and autocrafting

When an autocrafting task starts, its output resource is automatically added to the pinned resources row. If the resource is already pinned, the existing pin is enhanced with autocrafting status information instead of creating a duplicate entry.

While the task is running, hovering over the resource displays its crafting progress.

After the task completes, the resource remains visible in the pinned row until the {doctitle} is reopened. Temporary autocrafting pins are removed when the {doctitle} is reopened, while manually pinned resources remain pinned.
