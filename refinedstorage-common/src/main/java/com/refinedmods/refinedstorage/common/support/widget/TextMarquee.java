...
public class TextMarquee {
    ...
    public TextMarquee(final Component text, final int maxWidth) {
        this(text, maxWidth, 4210752, false, Style.NORMAL);
    }
    ...
    public void updateStateAndRender(final GuiGraphics graphics, final int x, final int y, final Font font,
        ...
        switch (style) {
            ...
            case NORMAL -> graphics.drawString(font, text, x + correctedOffset, y, color, dropShadow);
        }
        if (overflow) {
    ...
    private void renderSlot(final GuiGraphics graphics, final int x, final int y, final boolean large,
                            final Font font, final boolean alignRight) {
        graphics.pose().pushPose();
        // Large amounts overlap with the slot lines (see Minecraft behavior)
        graphics.pose().translate(x + (large ? 1F : 0F), y + (large ? 1F : 0F), 350);
        if (!large) {
        ...
        graphics.drawString(font, text, offsetX, large ? 8 : 22, color, true);
