...
public class ItemGridEventHandlerImpl implements ItemGridEventHandler {
    ...
    public void onScroll(Rs2ItemStack template, int slot, GridScrollMode mode) {
        ...
        switch (mode) {
            case GRID_TO_INVENTORY -> handleExtractFromGridToInventory(template, slot);
            case GRID_TO_CURSOR -> handleExtractFromGridToCursor(template);
            case INVENTORY_TO_GRID -> handleExtractFromInventoryToGrid(template, slot);
        }
    }
